...
Kernel permissions are the graph's authoritative resource checks. A handler `authorize` hook is an
additive business gate based on validated params, resolved `self`, caller identity, or domain state.
Because a handler's kernel authority composes caller and function grants, caller-sensitive operations
should explicitly `kernel.auth.require({ who: principal,... })` in `authorize`. See composed versus
caller authority.
