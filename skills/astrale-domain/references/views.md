...
with the shell client, giving it navigation and a session, while `none` serves it as a bare page. A view
that mounts a single-page app defaults to `shell`; one that just renders a response defaults to `none`.
Choose `shell` when the view should feel part of the surrounding app.
...
## Path: View Resolution

**View Resolution** is how a view path (`/:origin:view.name`) is followed to the view node it names: the
kernel walks from the domain to the member, arriving at the view that carries its URL. It is the
addressing counterpart of the runtime resolve that lists a node's views. One names a specific view to
open; the other asks what views a node has.
