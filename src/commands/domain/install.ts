...
/** Mirrors the kernel's `function.installDomain` return. */
...
Behavior:
  ...
  --direct: installs a url STRAIGHT onto the instance kernel (function.installDomain),
...
/**
 * Install a url straight onto the instance kernel (`function.installDomain`),
...
async function installDirect(target: string | undefined, opts: InstallOpts): Promise<void> {
  ...
  await runKernelCommand<DirectInstallResult>({
    ...
    fn: async (ctx) =>
      (await ctx.client.call(K.$.f('installDomain').path.domain.raw, {
        url,
