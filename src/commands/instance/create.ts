...
export function createInstanceCreateCommand(dependencies = { provisionInstance }) {
  return {
    name: 'create',
...
`,
    ...
    action: async (id: string | undefined, opts: ProvisionOpts) => {
      try {
        ...
        const { created, access, selectionError } = await dependencies.provisionInstance(id, opts)
...
export default createInstanceCreateCommand()
