...
flowchart LR
  ...
  T --> H[HostSession]
  C[CLI credential sources] --> R[HostAuth]
  R --> H
  H --> G[GraphApi]
  H --> A[AuthApi]
  H --> X[Command action]
...
Host owns source admission, Publication discovery, redirect witnessing, route caching, transport
selection, and the single safe stale-route recovery. The CLI does not reproduce those rules or
discover destination identity. `HostAuth.resolve(call, signal)` returns source-Kernel authority and
Host supplies the audience-free self Delegate used by the canonical path-only protocol. No
...
`HostAuth` capability. It cannot be combined with `--as` or `--creds`; contradictory selections fail
