...
describe('studio test gate', () => {
  test('the root and Studio suites include all client, server, and shared tests', () => {
    expect(cliPackage.scripts?.test).toBe(
      'node scripts/qualification/source-boundary.mjs --target && pnpm run check:cohort && bun test src studio/client/src studio/server studio/shared && node --test scripts/*.test.mjs scripts/cohort/__tests__/*.test.mjs',
    )
