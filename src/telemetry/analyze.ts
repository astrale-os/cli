...
FILE the issues that clear the quality bar (after writing report.md). Use the normal door, one call per issue:
  astrale --ci --no-prompt call "@$ASTRALE_ISSUES_PROJECT_ID::issues.astrale.ai:class.Project.method.createIssue" -i admin --data '{"title":"<one line>","description":"<evidence: exact command, exact error, expected vs actual, session ${id}>","priority":2}' --json
